#!/usr/bin/env python3
"""
Demo: Drive a rover around the compound perimeter wall.

Traces the inner edge of the 120m x 80m compound wall (2m inset).
...
# Compound origin (matches compound_ops.sdf spherical_coordinates)
ORIGIN_LAT = 32.990
ORIGIN_LON = -106.975
M_PER_DEG_LAT = 111132.92
M_PER_DEG_LON = 111132.92 * math.cos(math.radians(ORIGIN_LAT))


def local_to_latlon(x_east, y_north):
...
# Compound walls: x=±60, y=±40 (120m x 80m). 2m inset from each wall.
MARGIN = 2
PERIMETER_WAYPOINTS = [
    local_to_latlon(-60 + MARGIN, -40 + MARGIN),  # SW corner
    local_to_latlon(-60 + MARGIN,  40 - MARGIN),  # NW corner
    local_to_latlon( 60 - MARGIN,  40 - MARGIN),  # NE corner
    local_to_latlon( 60 - MARGIN, -40 + MARGIN),  # SE corner
    local_to_latlon(-60 + MARGIN, -40 + MARGIN),  # back to SW (close loop)
]
...
def main():
    ...
    print(f"  Perimeter: {len(waypoints)} waypoints (SW → NW → NE → SE → SW)")
